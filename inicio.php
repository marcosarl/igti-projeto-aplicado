<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
	<link rel="stylesheet" type="text/css" href="css/estilo.css">
    <title>RDIJ - Repositório de Documentação e
Informação Jurídica</title>
</head>
<body>
    	<div id="area-cabecalho">

		<div id="area-logo">
<center>
<Table>
    <th><img width="150" height="100" src="https://gidjsp.com.br/wp-content/uploads/2020/08/Logo_GIDISP_full-300x215.png" class="attachment-medium size-medium" alt="O GIDJ/SP" loading="lazy" srcset="https://gidjsp.com.br/wp-content/uploads/2020/08/Logo_GIDISP_full-300x215.png 300w, https://gidjsp.com.br/wp-content/uploads/2020/08/Logo_GIDISP_full-1024x733.png 1024w, https://gidjsp.com.br/wp-content/uploads/2020/08/Logo_GIDISP_full-768x550.png 768w, https://gidjsp.com.br/wp-content/uploads/2020/08/Logo_GIDISP_full-1536x1100.png 1536w, https://gidjsp.com.br/wp-content/uploads/2020/08/Logo_GIDISP_full-2048x1467.png 2048w, https://gidjsp.com.br/wp-content/uploads/2020/08/Logo_GIDISP_full.png 1600w" sizes="(max-width: 300px) 100vw, 300px" title="O GIDJ/SP" /></a><td>
    <th><td>
    <th><td>
    <th><td>
    <th><h1>Repositório de Documentação e <br>Informação Jurídica - RDIJ</h1><td>
</Table>
</center>
		</div>

		<div id="area-menu">
               </div>
 <center><a href="inicio.php">HOME</a>  |  <a href="cadastrar.php">CADASTRAR</a>  |  <a href="pesquisar.php">PESQUISAR</a>  |  <a href="listaUsuario.php">ADMIN</a>  |  <a href="index.html">SAIR</a></center> 
             </div>
                <div id="area-principal"><!-- inicio conteudo -->
		
		<div id="area-postagens">

		<div class="postagem"></div>
                <hr>
                <center> <h2> Página Inicial </h2> </center>
                <hr>
<br>
<h3>Bem Vindo!</h3>
<br>
Repositório de Documentação e Informação Jurídica - RDIJ
<br><br>
<h3>Constitue objetivo do RDIJ:</h3>
<br>
Organizar a troca de materiais entre os membros do GIDJSP - Grupo de Informação e Documentação Jurídica de São Paulo. 
<br><br>
<h3>Por meio do RDIJ, você pode:</h3>
<br>
<li><strong>Cadastrar: </strong> Cadastra o material, artigo, capítulo de livro.</li>
<li><strong>Pesquisar: </strong> Pesquisa um material na base utilizando um termo simples. A base retorna o material pelo título da obra.</li> 
<li><strong>Editar: </strong> Edita um material já cadastrado na base.</li>
<br>
<p class="text-muted pt-5">
                Aplicação desenvolvida por<a href="https://marcosarl.com">Marcos Antonio R. Lima</a>como resultado do projeto aplicado do MBA em Inovação e Transformação Digital do <span title="Instituto de Gestão e Tecnologia da Informação">IGTI</span>.
            </p><br>
            <p><a class="github-button text-muted" href="" data-size="large" aria-label="Star marcosarl/igti-projeto-aplicado on GitHub">Código-fonte disponível no Github</a></p>
        </div>
    </div>
</div>
<br>
<script async defer src="https://buttons.github.io/buttons.js"></script>

  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
  <script type="text/javascript" src="//maxcdn.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"></script>
                  </div><!-- fim conteudo -->
	</div>

	<div id="rodape">
		2021 - marcosarl.com - Todos os direitos reservados.
	</div>

</body>
</html>